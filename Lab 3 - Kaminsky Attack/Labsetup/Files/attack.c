...
void send_dns_request(char * IP_PKT, int pkt_size);
void send_dns_response(char * IP_PKT, int pkt_size);
...
void send_dns_request(char * IP_PKT, int pkt_size)
{
  ...
  send_raw_packet(IP_PKT, pkt_size);
}
...
void send_dns_response(char * IP_PKT, int pkt_size)
{
  ...
  for (unsigned short tid = 0; tid < 65535; tid++)
  {
    unsigned short tid_net_order;

    tid_net_order = htons(tid);
    memcpy(IP_PKT+ 28, &tid_net_order, 2);
    send_raw_packet(IP_PKT, pkt_size);
  }
} 

...
static struct nf_hook_ops PRE_ROUTING_HOOK, LOCAL_IN_HOOK, FORWARD_HOOK, LOCAL_OUT_HOOK, POST_ROUTING_HOOK; 
...
int registerFilter(void) {
   printk(KERN_INFO "Registering filters from printFilters.c.\n");

   PRE_ROUTING_HOOK.hook = printInfo;
   ...
   LOCAL_IN_HOOK.hook = printInfo;
   LOCAL_IN_HOOK.hooknum = NF_INET_LOCAL_IN;
   LOCAL_IN_HOOK.pf = PF_INET;
   LOCAL_IN_HOOK.priority = NF_IP_PRI_FIRST;
   nf_register_net_hook(&init_net, &LOCAL_IN_HOOK);

   FORWARD_HOOK.hook = printInfo;
   FORWARD_HOOK.hooknum = NF_INET_FORWARD;
   FORWARD_HOOK.pf = PF_INET;
   FORWARD_HOOK.priority = NF_IP_PRI_FIRST;
   nf_register_net_hook(&init_net, &FORWARD_HOOK);

   LOCAL_OUT_HOOK.hook = printInfo;
   LOCAL_OUT_HOOK.hooknum = NF_INET_LOCAL_OUT;
   LOCAL_OUT_HOOK.pf = PF_INET;
   LOCAL_OUT_HOOK.priority = NF_IP_PRI_FIRST;
   nf_register_net_hook(&init_net, &LOCAL_OUT_HOOK);

   POST_ROUTING_HOOK.hook = printInfo;
...
void removeFilter(void) {
   printk(KERN_INFO "printFilters.c filters are being removed.\n");
